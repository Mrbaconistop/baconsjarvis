      ...
      POST: async ({ request }) => {
        ...
        try {
          ...
          const tools = {
            ...
            recall_memory: tool({
              description: "Recall past conversations or messages based on a query.",
              inputSchema: z.object({
                query: z.string(),
            ...
            create_browser_tab: tool({
              description: "Create a new custom tab with a built-in web browser.",
              inputSchema: z.object({
                label: z.string().min(1).max(40),
                icon: z.string().max(40).nullable().optional().default("Globe"),
                description: z.string().max(300).nullable().optional(),
                home_url: z.string().url().optional().default("https://www.google.com"),
            ...
            create_custom_tab: tool({
              description: "Create a new custom tab with arbitrary HTML/CSS/JS.",
              inputSchema: z.object({
                label: z.string().min(1).max(40),
            ...
            update_custom_tab: tool({
              description: "Update a custom tab.",
              inputSchema: z.object({
                id: z.string().uuid().nullable().optional(),
            ...
            get_custom_tab: tool({
              description: "Get full HTML/content of a custom tab.",
              inputSchema: z.object({
                id: z.string().uuid().nullable().optional(),
                slug: z.string().nullable().optional(),
              }),
            ...
            get_place_address: tool({
              description: "Get the full address of a saved place by its label.",
              inputSchema: z.object({
                label: z.string(),
              }),
              execute: async ({ label }) => {
                ...
                try {
                  ...
                  if (!result) return { ok: false, error: "Could not reverse‑geocode this location." };
            ...
            update_profile: tool({
              ...
              inputSchema: z.object({
                ...
                preferred_briefing_time: z.string().optional(),
              }),
            ...
            get_file_url: tool({
              description: "Get a public URL to download a file.",
            ...
            // ---- Check‑ins ----
            log_checkin: tool({
              description: "Log or update today's daily check‑in.",
              inputSchema: z.object({
            ...
            list_briefing_webhooks: tool({
              description: "List configured Discord briefing webhooks.",
              inputSchema: z.object({}),
              execute: async () => {
                const { data } = await supabase
            ...
            configure_briefing: tool({
              description: "Toggle sections on a Discord briefing webhook.",
              inputSchema: z.object({
                id: z.string().uuid().nullable().optional(),
            ...
            // ---- System Status ----
            system_status: tool({
              description: "Get live system information: time, profile, counts, etc.",
              inputSchema: z.object({}),
              execute: async () => {
            ...
            navigate_app: tool({
              description: "Navigate the user's browser to a route in the app.",
              inputSchema: z.object({
            ...
            list_chat_threads: tool({
              description: "List the user's chat threads.",
          ...
          try {
            ...
            return result.toUIMessageStreamResponse({
              ...
              onError: (error: unknown) => {
                ...
                return `Signal interrupted, Sir. Copy this debug block if needed: ${JSON.stringify(payload)}`;
              },
            });
          } catch (error) {
