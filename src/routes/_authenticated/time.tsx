...
export const Route = createFileRoute("/_authenticated/time")({
  head: () => ({ meta: [{ title: "Time — JARVIS" }, { name: "description", content: "Reminders and calendar agenda." }] }),
  component: TimePage,
});
...
function TimePage() {
  ...
  return (
    <div className="flex flex-col h-screen">
      <PageHeader tag="01 · TIME" title="Schedule & reminders" subtitle="Pulled from your calendar and inbox, plus what you tell me." />
      <div className="flex-1 overflow-y-auto px-8 py-6 space-y-6">
        ...
        <section>
          <h2 className="font-display text-lg mb-3 flex items-center gap-2"><Calendar size={16} className="text-arc" /> Upcoming</h2>
          <div className="space-y-2">
            {upcoming.length === 0 && <div className="glass rounded-lg p-4 text-sm text-muted-foreground">Clear horizon, Sir.</div>}
            {upcoming.map((r: any) => {
              const mins = minutesUntil(r.datetime);
              const imminent = mins > 0 && mins <= 30;
              return (
                <article key={r.id} className={`${r.priority === "critical" ? "glass-critical" : "glass-strong"} rounded-lg p-4 flex items-start gap-4`}>
                  <button
                    onClick={async () => { await toggle({ data: { id: r.id, completed: true } }); qc.invalidateQueries({ queryKey: ["reminders"] }); }}
                  ...
                  <button
                    onClick={async () => { await del({ data: { id: r.id } }); qc.invalidateQueries({ queryKey: ["reminders"] }); }}
        ...
        {done.length > 0 && (
          <section>
            <h2 className="font-display text-lg mb-3 text-hud-dim flex items-center gap-2"><Check size={16} /> Completed</h2>
            <div className="space-y-2 opacity-60">
              {done.slice(0, 10).map((r: any) => (
                <article key={r.id} className="glass rounded-lg p-3 flex items-center gap-3">
                  <button
                    onClick={async () => { await toggle({ data: { id: r.id, completed: false } }); qc.invalidateQueries({ queryKey: ["reminders"] }); }}
