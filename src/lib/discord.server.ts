...
async function emailLines(): Promise<string[]> {
  try {
    const list = await gw(
      `${GMAIL}/users/me/messages?maxResults=8&q=${encodeURIComponent("newer_than:1d in:inbox -category:promotions -category:social")}`,
    );
...
async function calendarLines(): Promise<string[]> {
  try {
    ...
    const data = await gw(
      `${GCAL}/calendars/primary/events?timeMin=${now.toISOString()}&timeMax=${end.toISOString()}&singleEvents=true&orderBy=startTime&maxResults=10`,
    );
